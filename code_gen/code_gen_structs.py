...
def gen_types(types: dict) -> str:
    ...
    for name, type in types.items():
        ...
        contents.append(
            f'v => Err(Error::DoesNotCorespondToVariant(format!("Value {"{v}"} does not corespond to a variant in {name}"))),'
        )
        contents.append("}}")
...
def gen_method_enum(method: dict) -> str:
    ...
    for param in method["inParams"]:
        contents.append(
            f"{clean_name(param['name'])}: {convert_type_name(param['type'])},\n"
        )
    contents.append("},")
    ...
    contents.append(
        ", ".join(
            [
                f"{clean_name(a['name'])}: {convert_type_name(a['type'])}"
                for a in method["inParams"]
            ]
        )
        + ")"
    )
    ...
    contents.append(
        f"Self::inParams{'{'}{', '.join([clean_name(a['name']) for a in method['inParams']])}{'}'}"
    )
    contents.append("}")
    contents.append("}")
    ...
    contents.append(
        f"Msgtype::new({method['protocol'][0]['value']}, {method['protocol'][1]['value']}) {'}'}\n"
    )
